...
// ─── Pure logic (exported, safe to unit-test) ───────────────────────────────
// Factored out from the global.saveResults/global.scoreOne side effects below
// (2026-08-22, when this got wired into the automated pipeline as its own
// step for the first time — see CLAUDE.md's "Automated Claude scoring"
// section) so the actual save/combine logic has real test coverage instead
// of only being exercised by hand in an interactive session.

function buildDistribution(scoredGrants) {
  const byRec = {};
...
function saveScoredGrants(scoredGrants, profile) {
  ...
  return { scoredFile, tsvPath, reportPath, htmlPath, distribution: buildDistribution(scoredGrants) };
}
...
// ─── Orchestration (only runs when invoked directly, not on require()) ─────
// org-profile.yaml is gitignored/per-org and grants_prescored.json only
// exists after a scan — reading either unconditionally at module scope would
// make this file unsafe to require() from a test. Mirrors the same fix
// applied to near-miss-check.js.
if (require.main === module) {
  ...
  global.saveResults = function saveResults(scoredGrants) {
    const { scoredFile, tsvPath, reportPath, htmlPath, distribution } = saveScoredGrants(scoredGrants, profile);
...
module.exports = { saveScoredGrants, scoreOneGrant, buildDistribution };
