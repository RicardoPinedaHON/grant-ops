  ...
  try {
    ...
    for (const item of items) {
      ...
      grants.push({
        ...
        title: item.title,
        description: item.description || 'Inter-American Foundation grant for LAC grassroots organizations. IAF funds $25,000–$400,000 for community-led development projects in Honduras and the Caribbean.',
        url: item.url,
        funder: 'Inter-American Foundation (IAF)',
        deadline: item.deadline ? parseDate(item.deadline) : 'rolling',
        amount_min: amountMax ? null : 25000,
        amount_max: amountMax || 400000,
        country: 'Honduras',
  ...
  try {
    ...
    if (items.length === 0) {
      ...
      grants.push({
        ...
        description: 'The UNDP GEF Small Grants Programme provides grants up to $50,000 to community-based organizations and NGOs in Honduras working on biodiversity, climate change, land degradation, and chemicals. Supports indigenous peoples, youth, and women\'s groups. Applications reviewed year-round through the National Coordinator.',
  ...
  try {
    ...
    if (grants.length === 0) {
      grants.push({
        ...
        description: 'Rainforest Trust funds conservation projects in Central America including Honduras. Two grant types: Feasibility Awards ($10k–$30k for feasibility studies) and Protected Area Creation Awards ($50k–$500k for full projects). Supports indigenous and community-led conservation in La Mosquitia and other key landscapes. Three annual deadlines: March 1, July 1, October 1.',
        url: 'https://www.rainforesttrust.org/get-involved/apply-for-funding/',
        funder: 'Rainforest Trust',
        deadline: '2026-07-01',
        amount_min: 10000,
  ...
  try {
    ...
    for (const item of items) {
      grants.push({
        ...
        deadline: item.deadline ? parseDate(item.deadline) : 'rolling',
        amount_min: null,
        amount_max: parseAmountMax(item.amount) || null,
        country: 'Global',
    ...
    if (grants.length === 0) {
      ...
      grants.push({
        ...
        description: 'Climate Calling (by Climate Cardinals) curates fellowships, grants, and leadership programs for young climate leaders globally, including LAC. Grants range from $5,000–$100,000 depending on the program. Updated continuously with new open calls.',
